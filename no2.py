...
df = pd.read_csv("https://gml.noaa.gov/webdata/ccgg/trends/n2o/n2o_mm_gl.csv", header=45)
df["average_diff12"] = df["average"].diff(12)
df["average_diff12_rollmean12"] = df["average"].diff(12).rolling(12).mean()

fig, ax = plt.subplots()
ax.set_title("Global Monthly Mean Atmospheric $NO_{2}$ - Yearly Growth Rate")
